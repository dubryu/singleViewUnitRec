...
//	Assignment
	...
	{
		mFormatID = kAudioFormatLinearPCM;
		int sampleSize = SizeOf32(UInt32);
	...
	bool	IsCanonical() const
	{
		...
		bool interleaved = (mFormatFlags & kAudioFormatFlagIsNonInterleaved) == 0;
		unsigned sampleSize = SizeOf32(UInt32);
		reqFormatFlags = kAudioFormatFlagIsPacked;
	...
	void	SetAUCanonical(UInt32 nChannels, bool interleaved)
	{
		...
		mFormatFlags = kAudioFormatFlagIsPacked | (kAudioUnitSampleFractionBits << kLinearPCMFormatFlagsSampleFractionShift);
#else
		mFormatFlags = kAudioFormatFlagIsPacked;
#endif
		mChannelsPerFrame = nChannels;
		mFramesPerPacket = 1;
		mBitsPerChannel = 8 * SizeOf32(UInt32);
		if (interleaved)
			mBytesPerPacket = mBytesPerFrame = nChannels * SizeOf32(UInt32);
		else {
			mBytesPerPacket = mBytesPerFrame = SizeOf32(UInt32);
            mFormatFlags |= kAudioFormatFlagIsPacked;
        }
    }
